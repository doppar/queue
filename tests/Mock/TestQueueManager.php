...
class TestQueueManager
{
     ...
     *
     * @param string $queue
     * @return MockQueueJob|null
     */
    public function pop(string $queue = 'default'): ?MockQueueJob
    {
        try {
            $job = MockQueueJob::available($queue)->first();
     ...
     *
     * @param MockQueueJob $queueJob
     * @return bool
     */
    public function delete(MockQueueJob $queueJob): bool
    {
        try {
     ...
     * @return bool
     */
    public function release(MockQueueJob $queueJob, int $delay = 0): bool
    {
        try {
    ...
    {
        return MockQueueJob::where('queue', $queue)
    ...
    {
        return MockQueueJob::where('queue', $queue)->delete();
    }
